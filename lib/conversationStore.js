...
export async function deleteConversationSession(sessionId) {
  const result = await redis(['lrange', LOG_KEY, '0', String(MAX_LOGS - 1)])
  const matchingLogs = (result.result || []).filter((item) => {
    try {
      return JSON.parse(item).sessionId === sessionId
    } catch {
      return false
    }
  })

  for (const item of matchingLogs) await redis(['lrem', LOG_KEY, '0', item])
